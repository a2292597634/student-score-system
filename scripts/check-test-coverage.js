...
const MODULES_DIR = path.join(PROJECT_ROOT, 'src');
const TESTS_DIR = path.join(PROJECT_ROOT, 'tests');
const MIN_TESTS_PER_FILE = 3;
const EXCLUDED_MODULES = ['main'];
...
function checkModuleCoverage() {
  ...
  if (missing.length > 0) {
    ...
    for (const m of missing) {
      console.log(`  ${C.red('✗')} src/${m}.js → 缺失 tests/${m}.test.js`);
    }
  } else {
...
function checkTestCount(covered) {
  ...
  if (exists(integrationDir)) {
    const intFiles = fs.readdirSync(integrationDir).filter((f) => f.endsWith('.test.js'));
    for (const f of intFiles) {
      const p = path.join(integrationDir, f);
      const count = countTests(p);
      if (count < MIN_TESTS_PER_FILE) {
        lowCount.push({ name: `integration/${path.basename(f, '.test.js')}`, count });
        console.log(`  ${C.yellow('⚠')} tests/integration/${f} — ${count} 个用例（要求 ≥${MIN_TESTS_PER_FILE}）`);
      } else {
        console.log(`  ${C.green('✓')} tests/integration/${f} — ${count} 个用例`);
      }
    }
  }
...
function printSummary(results) {
  ...
  console.log(`执行验证: ${execution.ok ? C.green(`通过 ✅ (${execution.passed || '?'} 个测试)`) : C.red('未通过 ❌')}`);
