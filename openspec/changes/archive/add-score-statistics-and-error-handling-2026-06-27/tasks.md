...
- [x] 2.1 实现 `src/main.js` 完整的 readline 菜单循环
- [x] 2.2 实现录入交互子流程（逐项输入学号、姓名、三科成绩）
- [x] 2.3 实现查询交互子流程（按学号查询 / 查看全部）
- [x] 2.4 实现统计交互子流程（按学号查看统计 / 班级统计）
- [x] 2.5 实现全局容错（非法菜单、非法成绩输入等场景）
- [x] 2.6 确保录入后自动持久化

## 3. 回归验证

- [x] 3.1 运行 `npm test` 确认全部测试通过
- [x] 3.2 手动运行 `node src/main.js` 验证完整 CLI 交互流程
- [x] 3.3 验证程序重启后数据不丢失
- [x] 3.4 验证各种非法输入不会导致程序崩溃
- [x] 3.5 重读 Change 4 的 proposal、design、specs 和 tasks，确认实现未越界
...
- [x] **全部测试通过**：运行 `npm test` 输出全部通过，零失败
- [x] **手动验证完成**：手动运行 CLI 验证完整交互流程
- [x] **无测试作弊**：没有 mock 掉核心业务逻辑

### 阶段四：归档阶段

- [x] **测试文件已提交**：测试文件纳入 git 版本控制
